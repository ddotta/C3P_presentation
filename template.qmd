...
3. **Auto-join session** : Si l'option est sélectionnée, votre navigateur vous redirige automatiquement vers votre nouvelle session dès qu'elle est prête. (cas d'usage classique)  
...
[Règle : Tout script soumis en Workbench Job doit être 100 % autonome.]{.content-box-red}  
[Le job s'exécute dans une session R totalement isolée (souvent dans un conteneur ou sur un serveur distant). Il n'accède pas aux variables, fonctions ou données chargées dans la console RStudio active.]{.content-box-red}
